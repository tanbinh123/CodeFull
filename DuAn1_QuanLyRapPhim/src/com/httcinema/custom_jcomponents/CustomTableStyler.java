...
public class CustomTableStyler extends AbstractCellEditor implements TableCellRenderer, TableCellEditor, ActionListener, MouseListener {
	 ...
	 */
	public CustomTableStyler(JTable table, Action action, int column, Color selectedBackground) {
		this(table, action, column, table.getSelectionBackground(), table.getSelectionBackground());
	}
	
	public CustomTableStyler(JTable table, Action action, int column) {
		this(table, action, column, table.getSelectionBackground(), table.getSelectionBackground());
	}
	
	public CustomTableStyler(JTable table, Action action, int column, Color selectedBackground, Color clickedBackground) {
		this.table = table;
		this.action = action;
		...
		renderButton.setBackground(clickedBackground);
		
		editButton = new CustomButton();
		editButton.setBackground(clickedBackground);
...
//		setFocusBorder( new LineBorder(Color.BLUE) );
	...
	public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
		if (value == null) {
			...
			editButton.setIcon(((JButton) value).getIcon());
		} else {
...
//
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		if (isSelected) {
			renderButton.setForeground(new Color(0,0,0,0));
			renderButton.setBackground(selectedBackground);
		} else {
			renderButton.setForeground(new Color(0,0,0,0));
			renderButton.setBackground(Theme.cellBackgroundColor);
		}
