...
public class QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog extends CustomDialog {
	...
	public static void showDialog(Component owner, TaiKhoan taiKhoan, int trangThai) {
		QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog dialog;
		if (owner instanceof JDialog) {
			dialog = new QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog((Dialog) owner);
		}
		else {
			dialog = new QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog((Frame) owner);
		}
	...
	public QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog(Frame owner) {
		super(owner);
	}
	
	public QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog(Dialog owner) {
		super(owner);
	}

	public QuanLyNhanVienJScrollPaneNoiDungJPanelThongTinChiTietJDialog() {
		setUndecorated(true);
		init(null, 1);
	}
	...
	public void init(TaiKhoan taiKhoan, int trangThai) {
		...
		txtTenDangNhap = new CustomTextField(30);
		...
		txtMatKhau = new CustomPasswordField(30);
		txtMatKhau.setFont(new Font("Tahoma", Font.PLAIN, 15));
		txtMatKhau.setBounds(170, 52, 312, 30);
		...
		lblChuThichNgay.setBounds(330, 175, 152, 30);
	...
	private void setState(TaiKhoan model) {
		if (trangThai == THEM) {
			vaiTro = (new VaiTroDAO().findById("NV"));
			
			lblTieuDe.setText("THÊM NHÂN VIÊN MỚI");
			...
			vaiTro = (new VaiTroDAO().findById(model.getMaVaiTro()));
	...
	private void insert() {
		...
		try {
			if (imageFile != null) {
				...
				lblHinh.setToolTipText(model.getTenDangNhap() + ShareHelper.getFileExtension(imageFile));
			}
	...
	private boolean isValidData() {
		...
		if (DateHelper.toDate(txtNgaySinh.getText().trim(), "dd-MM-yyyy") == null) {
			DialogHelper.warning(this, "Định dạng ngày phải là dd-MM-yyyy");
			txtNgaySinh.requestFocus();
			return false;
		}
		
		if (DateHelper.calculateAge(DateHelper.toDate(txtNgaySinh.getText().trim(), "dd-MM-yyyy")) < 16) {
