...
public class PhimControllInsert extends javax.swing.JDialog {
    ...
    private boolean check() {
        ...
        String ngayCongChieu = txtNgayCongChieu.getText();
        String regexInt = "^\\d+"; // có thể có một hoặc nhiều số
        String regexDate = "\\d{4}-\\d{1,2}-\\d{1,2}";
        if (tenPhim.isEmpty() || nhaSanXuat.isEmpty() || doTuoi.isEmpty() || thoiLuong.isEmpty() || ngayCongChieu.isEmpty()) {
            SonOptionPane.showMessage(super.getParent(), "Không được để trống!", SonOptionPane.WARNING);
            kiemTra = false;
        } else if (!doTuoi.matches(regexInt)) {
            SonOptionPane.showMessage(super.getParent(), "VUI LÒNG NHẬP ĐÚNG ĐỊNH DẠNG", SonOptionPane.INFO);
            txtDoTuoi.setText("");
            txtDoTuoi.requestFocus();
            kiemTra = false;
        } else if (!thoiLuong.matches(regexInt)) {
            SonOptionPane.showMessage(super.getParent(), "VUI LÒNG NHẬP ĐÚNG ĐỊNH DẠNG", SonOptionPane.INFO);
            txtThoiLuong.setText("");
            txtThoiLuong.requestFocus();
            kiemTra = false;
        } else if (!ngayCongChieu.matches(regexDate)) {
            SonOptionPane.showMessage(super.getParent(), "VUI LÒNG NHẬP ĐÚNG ĐỊNH DẠNG", SonOptionPane.INFO);
